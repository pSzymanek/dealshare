...
export function PartnerRequestForm() {
  ...
  async function submit(event: FormEvent<HTMLFormElement>) {
    ...
    setMessage(data.message ?? (response.ok ? "Dziękujemy. Otrzymaliśmy informacje o Twojej firmie." : "Nie udało się wysłać formularza. Spróbuj ponownie."));
  }

  if (status === "success") return <div className="border-y border-teal/30 bg-teal/5 py-10 text-center"><CheckCircle2 className="mx-auto text-teal" size={42} /><h2 className="mt-4 text-2xl font-black text-navy">Dziękujemy za przedstawienie oferty</h2><p className="mx-auto mt-3 max-w-xl text-sm leading-7 text-slate-600">{message}</p></div>;
  ...
  return (
    <form onSubmit={submit} className="grid gap-5" noValidate>
      ...
      <fieldset><legend className="text-sm font-bold text-navy">W jakich obszarach pomagasz firmom? *</legend><div className="mt-3 flex flex-wrap gap-2">{categoryOptions.map((category) => { const active = values.categories.includes(category); return <button key={category} type="button" onClick={() => update('categories', active ? values.categories.filter((item) => item !== category) : [...values.categories, category])} className={`rounded border px-3 py-2 text-sm font-bold ${active ? 'border-electric bg-electric/5 text-electric' : 'border-slate-200 text-slate-600'}`}>{category}</button>; })}</div></fieldset>
      <label><span className="text-sm font-bold text-navy">Opowiedz o swojej ofercie *</span><textarea required minLength={40} maxLength={10000} value={values.offerDescription} onChange={(event) => update('offerDescription',event.target.value)} className="mt-2 min-h-40 w-full rounded-md border border-slate-300 px-4 py-3 text-sm leading-6 outline-none focus:border-electric focus:ring-4 focus:ring-electric/10" placeholder="Komu pomagasz, jaki problem rozwiązujesz i co wyróżnia Twoją ofertę?" /></label>
      ...
      <button type="submit" disabled={status === 'loading'} className="inline-flex min-h-11 w-fit items-center gap-2 rounded-md bg-electric px-6 text-sm font-bold text-white hover:bg-navy disabled:opacity-60"><Send size={17} />{status === 'loading' ? 'Wysyłamy...' : 'Porozmawiajmy o współpracy'}</button>
    </form>
  );
}
