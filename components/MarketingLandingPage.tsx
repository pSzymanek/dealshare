...
export function MarketingLandingPage({ landing, offer }: MarketingLandingPageProps) {
  ...
  return (
    <main className="bg-white text-ink">
      <section className="relative isolate overflow-hidden border-b border-slate-200 bg-navy-gradient text-white">
        <div className="absolute inset-0 -z-10 bg-[linear-gradient(115deg,rgba(255,255,255,0.08),transparent_42%)]" />
        <Container className="py-12 sm:py-16 lg:py-20">
          <nav className="mb-8 flex flex-wrap gap-2 text-sm font-semibold text-white/70" aria-label="Breadcrumb">
            ...
            <span>/</span>
            <Link href="/oferty" className="hover:text-white">
              Oferty
            </Link>
            <span>/</span>
          ...
          <div className="grid gap-10 lg:grid-cols-[1fr_420px] lg:items-center">
            <div className="max-w-4xl">
              ...
              <p className="mt-8 text-sm font-black uppercase tracking-[0.22em] text-cyan">{landing.eyebrow}</p>
              <h1 className="mt-4 max-w-4xl text-4xl font-black tracking-tight sm:text-6xl lg:text-7xl">{landing.title}</h1>
              <p className="mt-6 max-w-3xl text-lg leading-8 text-white/76 sm:text-xl sm:leading-9">{landing.lead}</p>
            ...
            <aside className="reveal-on-load rounded-lg border border-white/15 bg-white/10 p-6 shadow-glow backdrop-blur">
              ...
              <h2 className="mt-6 text-2xl font-black tracking-tight">Co dostajesz na start</h2>
              <ul className="mt-5 grid gap-3">
                {landing.proofPoints.map((point) => (
                  <li key={point} className="flex gap-3 text-sm font-bold leading-6 text-white/82">
                    <span className="mt-0.5 grid h-6 w-6 shrink-0 place-items-center rounded-full border border-cyan/40 text-cyan">✓</span>
                    {point}
                  </li>
                ))}
              </ul>
        ...
        <Container>
          ...
          <div className="mt-8 grid gap-5 md:grid-cols-3">
            {landing.quickWins.map((item, index) => (
              <article key={item.title} className={`reveal-on-scroll reveal-delay-${Math.min(index + 1, 3)} rounded-lg border border-slate-200 bg-white p-6 shadow-sm`}>
      ...
      <section id="szczegoly" className="bg-mist py-14 sm:py-16">
        <Container>
          <div className="grid gap-8 lg:grid-cols-[320px_1fr]">
            ...
            <div className="reveal-on-scroll divide-y divide-slate-200 overflow-hidden rounded-lg border border-slate-200 bg-white shadow-sm">
              {landing.detailGroups.map((group, index) => (
                <details key={group.title} className="group p-5 sm:p-6" open={index === 0}>
                  <summary className="flex cursor-pointer list-none items-center justify-between gap-5 text-lg font-black text-navy">
                    {group.title}
                    <span className="grid h-9 w-9 shrink-0 place-items-center rounded-full bg-mist text-electric transition group-open:rotate-180">⌄</span>
                  </summary>
      ...
      <section className="bg-white py-14 sm:py-16">
        <Container>
          <div className="reveal-on-scroll rounded-lg bg-navy-gradient p-7 text-white shadow-glow sm:p-10 lg:flex lg:items-center lg:justify-between lg:gap-10">
            <div className="max-w-3xl">
              <h2 className="text-3xl font-black tracking-tight sm:text-4xl">{landing.conversionTitle}</h2>
              <p className="mt-4 text-base leading-8 text-white/74">{landing.conversionText}</p>
            </div>
      ...
      <LandingLeadForm offerSlug={offer.slug} offerTitle={offer.title} defaultCategory={offer.category} title={landing.formTitle} text={landing.formText} />
    </main>
  );
}
