...
export function CaseForm({ pathType = "guided_matching", offerSlug, offerTitle, defaultCategory = "", compact = false }: CaseFormProps) {
  ...
  async function submit(event: FormEvent<HTMLFormElement>) {
    ...
    if (!response.ok) {
      setStatus("error");
      setMessage(data.message ?? "Nie udało się utworzyć sprawy.");
      return;
    }

    setStatus("success");
    setCaseNumber(data.caseNumber ?? "");
    setMessage(data.message ?? "Sprawa została utworzona.");
  }
  ...
  if (status === "success") {
    return (
      <div className="border-y border-teal/30 bg-teal/5 py-10 text-center">
        ...
        <p className="mt-4 text-xs font-black uppercase tracking-[0.16em] text-teal">Sprawa utworzona</p>
        <h2 className="mt-2 text-3xl font-black text-navy">{caseNumber}</h2>
        <p className="mx-auto mt-3 max-w-xl text-sm leading-7 text-slate-600">{message} Na podany adres wysłaliśmy potwierdzenie oraz bezpieczny link do panelu.</p>
        <Link href="/logowanie" className="mt-6 inline-flex min-h-11 items-center rounded-md bg-electric px-5 text-sm font-bold text-white hover:bg-navy">Przejdź do panelu</Link>
      </div>
    );
  }
    ...
    <form onSubmit={submit} className={compact ? "grid gap-5" : "rounded-lg border border-slate-200 bg-white p-5 shadow-card sm:p-7"} noValidate>
      <div className="grid gap-5 sm:grid-cols-2">
        ...
        <label className="block">
          <span className="text-sm font-bold text-navy">Kategoria potrzeby *</span>
      ...
      <label className="mt-5 block">
        <span className="text-sm font-bold text-navy">Opisz sytuację firmy *</span>
        <textarea required minLength={20} maxLength={8000} value={values.description} onChange={(event) => update("description", event.target.value)} className="mt-2 min-h-36 w-full rounded-md border border-slate-300 px-4 py-3 text-sm leading-6 outline-none focus:border-electric focus:ring-4 focus:ring-electric/10" placeholder={offerTitle ? `Napisz, co chcesz ustalić w sprawie: ${offerTitle}` : "Opisz potrzebę, kontekst i oczekiwany rezultat."} />
      </label>

      <fieldset className="mt-5">
        <legend className="text-sm font-bold text-navy">Preferowany kontakt</legend>
      ...
      <button type="submit" disabled={status === "loading"} className="mt-6 inline-flex min-h-11 items-center justify-center gap-2 rounded-md bg-electric px-6 text-sm font-bold text-white transition hover:bg-navy disabled:opacity-60">
        <Send size={17} /> {status === "loading" ? "Tworzymy sprawę..." : "Utwórz sprawę"}
      </button>
    </form>
  );
}
