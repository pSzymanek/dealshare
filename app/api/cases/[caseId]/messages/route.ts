...
export async function POST(request: Request, { params }: { params: Promise<{ caseId: string }> }) {
  ...
  await db.transaction(async tx => {
    ...
    await tx.insert(caseEvents).values({ id: randomUUID(), caseId: caseData.id, actorUserId: context.session.user.id, eventType: "client_message", message: "Klient dodał wiadomość lub uzupełnienie." });
  });
